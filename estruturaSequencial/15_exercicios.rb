...
puts "
+ Salário Bruto : R$ #{sprintf("%.2f", grossSalary)}
- IR (11%) : R$ #{sprintf("%.2f", incomeTax(grossSalary))}
- INSS (8%) : R$ #{sprintf("%.2f", socialInsurance(grossSalary))}
- Sindicato ( 5%) : R$ #{sprintf("%.2f", laborUnion(grossSalary))}
= Salário Liquido : R$ #{sprintf("%.2f", ns)}
"