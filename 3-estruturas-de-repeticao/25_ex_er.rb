...
if (average >= 0) && (average <= 25)
  puts "Com uma média de idade de #{format('%.2f', average)} a turma é JOVEM"
elsif (average >= 26) && (average <= 60)
  puts "Com uma média de idade de #{format('%.2f', average)} a turma é ADULTA"
else
  puts "Com uma média de idade de #{format('%.2f', average)} a turma é IDOSA"
end
